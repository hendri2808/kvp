...
/// Types of asset supported by the kvp runtime.
pub enum AssetTypes {
...
// kvp only knows about one asset, the balances pallet.
...
impl WeighMultiAssets for MultiAssetFilter {
	fn weigh_multi_assets(&self, balances_weight: Weight) -> Weight {
		match self {
			...
			// We don't support any NFTs on kvp, so these two variants will always match
...
pub struct kvpXcmWeight<RuntimeCall>(core::marker::PhantomData<RuntimeCall>);
impl<RuntimeCall> XcmWeightInfo<RuntimeCall> for kvpXcmWeight<RuntimeCall> {
	...
	fn reserve_asset_deposited(assets: &MultiAssets) -> Weight {
		// kvp doesn't support ReserveAssetDeposited, so this benchmark has a default weight
	...
	fn exchange_asset(_give: &MultiAssetFilter, _receive: &MultiAssets, _maximal: &bool) -> Weight {
		// kvp does not currently support exchange asset operations
		Weight::MAX
	}
	...
	fn universal_origin(_: &Junction) -> Weight {
		// kvp does not currently support universal origin operations
		Weight::MAX
	}
	fn export_message(_: &NetworkId, _: &Junctions, _: &Xcm<()>) -> Weight {
		// kvp relay should not support export message operations
		Weight::MAX
	}
	fn lock_asset(_: &MultiAsset, _: &MultiLocation) -> Weight {
		// kvp does not currently support asset locking operations
		Weight::MAX
	}
