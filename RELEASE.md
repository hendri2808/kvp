...
### Notes
* The release-candidate branch *must* be made in the paritytech/kvp repo in
...
1. To initiate the release process:
  ...
  2. In the [substrate](https://github.com/paritytech/substrate) repo, check out the commit used by kvp (this can be found using the following command in the *kvp* repo: `grep 'paritytech/substrate' Cargo.lock | grep -E '[0-9a-f]{40}' | sort | uniq `
  3. Branch off this **substrate** commit into its own branch: `git branch -b kvp-v0.8.26; git push origin refs/heads/kvp-v0.8.26`
  4. In the **kvp** repository, use [diener](https://github.com/bkchr/diener/) to switch to this branch: `diener update --branch "kvp-v0.8.26" --substrate`. Update Cargo.lock (to do this, you can run `cargo build` and then ctrl+c once it finishes fetching and begins compiling)
  5. Push the **kvp** `release-v0.8.26` branch to Github: `git push origin refs/heads/release-v0.8.26`
...
3. NOACTION: A draft release and runtime WASMs are created for this
  ...
  the internal kvp matrix channel.
...
6. (optional) If a fix is required to the release-candidate:
  ...
  5. If there are **substrate** changes required, these should be cherry-picked to the substrate `kvp-v0.8.26` branch and pushed, and the version of substrate used in **kvp** updated using `cargo update -p sp-io`
...
released version of kvp, without taking *every* change to `master` since
