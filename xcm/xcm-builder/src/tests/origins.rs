...
fn universal_origin_should_work() {
	...
	// Parachain 2 may represent kvp to us
	add_universal_alias(Parachain(2), kvp);
...
fn export_message_should_work() {
	...
	// Local parachain #1 issues a transfer asset on kvp Relay-chain, transfering 100 Planck to
	// kvp parachain #2.
	...
	assert_eq!(
		exported_xcm(),
		vec![(kvp, 403611790, uni_src, Here, expected_message, expected_hash)]
	);
}
