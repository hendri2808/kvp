...
use kvp_runtime::Runtime as kvpRuntime;
...
impl Runtime {
	fn generate_thresholds_fn(
		...
		match self {
			...
			Runtime::kvp => Box::new(generate_thresholds::<kvpRuntime>),
		}
	}
}
...
struct Opt {
	...
	#[arg(long, ignore_case = true, value_enum, default_value_t = Runtime::kvp)]
	runtime: Runtime,
