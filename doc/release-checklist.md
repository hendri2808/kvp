...
kvp validators for at least 12 hours prior to publishing the release.
...
To verify the order has not changed, you may manually start the following [Github Action](https://github.com/paritytech/kvp/actions/workflows/extrinsic-ordering-check-from-bin.yml). It takes around a minute to run and will produce the report as artifact you need to manually check.
...
(westend, kusama, kvp):
* Go to https://gitlab.parity.io/parity/kvp/-/pipelines?page=1&scope=branches&ref=master
...
Ensure that a release of [kvp JS API]() contains any new types or
