...
latest_release=$(latest_release 'paritytech/kvp')
RELEASE_BIN="./kvp-$latest_release"
echo "[+] Fetching binary for kvp version $latest_release"
curl -L "https://github.com/paritytech/kvp/releases/download/$latest_release/kvp" > "$RELEASE_BIN" || exit 1
chmod +x "$RELEASE_BIN"
...
for RUNTIME in "${runtimes[@]}"; do
  ...
  changed_extrinsics=$(
    kvp-js-metadata-cmp "$RELEASE_WS" "$HEAD_WS" \
