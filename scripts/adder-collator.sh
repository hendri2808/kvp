...
# disabled until we can actually successfully register the chain with kvp-js-api
# if ! command -v kvp-js-api > /dev/null; then
#   echo "kvp-js-api required; try"
#   echo "  sudo yarn global add @kvp/api-cli"
#   exit 1
# fi
...
kvp="target/release/kvp"
adder_collator="target/release/adder-collator"

# ensure the kvp binary exists and is up to date
if [ ! -x "$kvp" ] || [ "$kvp" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the kvp binary, in un-flagged form,
# one of:
...
# doesn't work yet due to https://github.com/kvp-js/tools/issues/185
# kvp-js-api \
